...
fn port_base(port: Port) -> usize {
    ...
    match port {
        ...
        Port::H | Port::I => panic!("port unavailable on STM32G431VBT6"),
    }
}
unsafe fn enable_port(port: Port) {
    let rcc = unsafe { &*stm32g4::stm32g431::RCC::ptr() };
    rcc.ahb2enr().modify(|_, w| match port {
        ...
        Port::H | Port::I => unreachable!(),
    });
    let _ = rcc.ahb2enr().read().bits();
}
unsafe fn field(address: usize, shift: u32, width: u32, value: u32) {
    let p = address as *mut u32;
    let mask = ((1u32 << width) - 1) << shift;
    let current = unsafe { read_volatile(p) };
    unsafe { write_volatile(p, (current & !mask) | ((value << shift) & mask)) };
}
fn pull(p: Pull) -> u32 {
    match p {
        Pull::None => 0,
...
impl OutputPin {
    ...
    pub unsafe fn claim(pin: Pin, high: bool) -> Self {
        unsafe { enable_port(pin.port()) };
        let base = port_base(pin.port());
        let mask = 1u32 << pin.index();
        unsafe {
            write_volatile(
                (base + BSRR) as *mut u32,
                if high { mask } else { mask << 16 },
            );
            field(base + MODER, u32::from(pin.index()) * 2, 2, 1)
        }
        Self { base, mask, high }
    }
}
...
impl InputPin {
    /// # Safety
    /// Caller must exclusively own the pin.
    pub unsafe fn claim(pin: Pin, p: Pull) -> Self {
        unsafe { enable_port(pin.port()) };
        let base = port_base(pin.port());
        unsafe {
            field(base + MODER, u32::from(pin.index()) * 2, 2, 0);
            field(base + PUPDR, u32::from(pin.index()) * 2, 2, pull(p));
        }
        Self {
            base,
            mask: 1u32 << pin.index(),
...
impl AlternatePin {
    /// # Safety
    /// Caller must exclusively own the pin and choose a supported AF.
    pub unsafe fn claim(pin: Pin, config: AlternateConfig) -> Result<Self> {
        unsafe { enable_port(pin.port()) };
        let base = port_base(pin.port());
        let i = u32::from(pin.index());
        let (afr, shift) = if i < 8 {
            (AFRL, i * 4)
        } else {
            (AFRH, (i - 8) * 4)
        };
        ...
        unsafe {
            field(base + afr, shift, 4, u32::from(config.function.number()));
            field(
                base + OTYPER,
                i,
                1,
            ...
            field(base + OSPEEDR, i * 2, 2, speed);
            field(base + PUPDR, i * 2, 2, pull(config.pull));
            field(base + MODER, i * 2, 2, 2);
        }
        Ok(Self { pin, config })
    }
