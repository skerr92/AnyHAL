...
fn main() {
    ...
    println!("cargo:rerun-if-changed=../../resources/devices/microchip/atsamd51p19a-routes.csv");

    let out = PathBuf::from(env::var_os("OUT_DIR").expect("Cargo sets OUT_DIR"));
    fs::copy("memory.x", out.join("memory.x")).expect("copy SAMD51P19A linker script");
    println!("cargo:rustc-link-search={}", out.display());
    println!("cargo:rustc-link-arg=-Tmemory.x");

    let package = selected_package();
    let csv = fs::read_to_string("../../resources/devices/microchip/atsamd51p19a-routes.csv")
        .expect("read Microchip-derived ATSAMD51P19A route data");
...
fn selected_package() -> &'static str {
    let tfbga120 = env::var_os("CARGO_FEATURE_PACKAGE_TFBGA120").is_some();
    let tqfp128 = env::var_os("CARGO_FEATURE_PACKAGE_TQFP128").is_some();
    match (tfbga120, tqfp128) {
        (true, false) => "TFBGA120",
        (false, true) => "TQFP128",
        (false, false) => panic!(
            "select exactly one SAMD51P19A package feature: package-tfbga120 or package-tqfp128"
        ),
        (true, true) => panic!("SAMD51P19A package features are mutually exclusive"),
    }
}
...
fn generate_routes(package: &str, routes: &[Route]) -> String {
    let mut output = format!(
        "// Generated from Microchip SAMD51_DFP 3.9.276. Do not edit.\n\
    ...
    for peripheral in ["SERCOM1", "SERCOM4"] {
        output.push_str(&generate_fn(
            ...
            |route| {
                format!(
                    "SercomRoute {{ pad: {}, function: {}, ioset: {} }}",
